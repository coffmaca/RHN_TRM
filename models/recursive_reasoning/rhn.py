    ...
    def forward(self, activations: torch.Tensor) -> Tuple[dict, torch.Tensor]:
        ...
        for layer in self.config_per_layer:
            ...
            if self.config_per_layer[layer]["type"] == "vector":
                outputs_a = torch.tanh(rms_norm(outputs_a, variance_epsilon=self.config.rms_norm_eps) * 0.5)
                outputs_by_layer[layer] = outputs_a
            else:
                outputs_a = torch.tanh(rms_norm(outputs_a, variance_epsilon=self.config.rms_norm_eps) * 0.5)
                outputs_b = torch.tanh(rms_norm(outputs_b, variance_epsilon=self.config.rms_norm_eps) * 0.5)
