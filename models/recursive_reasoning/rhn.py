...
class RHN_ACTV1_Inner(nn.Module):
    ...
    def forward(self, carry: RHN_ACTV1InnerCarry, batch: Dict[str, torch.Tensor], log_deep_metrics: bool = False,
                **kwargs) -> Tuple[RHN_ACTV1InnerCarry, torch.Tensor, Tuple[torch.Tensor, torch.Tensor], torch.Tensor, torch.Tensor, dict]:
        seq_info = dict(
        ...
        for _L_step in range(self.config.L_cycles):
            prev_z_L = z_L
            z_L, step_l2, step_kl, step_m = self._dynamic_forward(z_L=z_L,
                                                z_H=z_H,
                                                input_embeddings=input_embeddings,
                                                log_deep_metrics=log_deep_metrics,
                                                **seq_info)
        ...
        prev_z_H = z_H
        z_H, step_l2, step_kl, step_m = self._dynamic_forward(z_L=z_L,
                                    z_H=z_H,
                                    input_embeddings=None,
                                    log_deep_metrics=log_deep_metrics,
                                    **seq_info)
        ...
        return new_carry, output, (q_logits[..., 0], q_logits[..., 1]), avg_l2, avg_kl, total_metrics

    def _dynamic_forward(self, z_L, z_H, input_embeddings=None, log_deep_metrics=False, **seq_info)-> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        return h_base + h_dyn, step_l2, step_kl, step_metrics
...
class RHN_ACTV1(nn.Module):
    ...
    def forward(self, carry: RHN_ACTV1Carry, batch: Dict[str, torch.Tensor], log_deep_metrics: bool = False) -> Tuple[RHN_ACTV1Carry, Dict[str, torch.Tensor]]:
        ...
        # Forward inner model
        new_inner_carry, logits, (q_halt_logits, q_continue_logits), hypernet_l2, hypernet_kl, deep_metrics = self.inner(new_inner_carry, new_current_data)
