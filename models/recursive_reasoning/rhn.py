        ...
        if self.attn:
            if self.config.mlp_t:
                ...
                self.self_attn = Attention(
                    hidden_size=config.hypernet_hidden_size + config.depth_enc_dim,
                    head_dim=(config.hypernet_hidden_size + config.depth_enc_dim) // config.num_heads,
...
class RHN_Hypernetwork(nn.Module):
    def __init__(self, config: RHN_ACTV1Config, layer_specs) -> None:
        ...
        self.input_size = self.config.hidden_size + self.config.depth_enc_dim
        ...
        self.output_head = CastedLinear(self.input_size,
...
class RHN_ACTV1_Inner(nn.Module):
    ...
    def _dynamic_forward(self, z_L, z_H, input_embeddings=None, log_deep_metrics=False, update_codebook=False, global_step=None,
        ...
        dynamic_weights, step_l2, step_vq = self.hypernet(hyper_input.detach(), update_codebook, **seq_info)
