...
class RHN_Hypernetwork(nn.Module):
    def __init__(self, config: RHN_ACTV1Config, layer_specs) -> None:
        ...
        for _i in range(self.config.H_layers):
            self.hypernet_base.append(RHN_ACTV1Block(self.config, attn=False, attn_type="perceiver", attn_params={
                "input_size": self.config.hypernet_hidden_size,
                "kv_size": self.input_size,
                "heads": self.config.perceiver_heads,
            })
...
class RHN_ACTV1_Inner(nn.Module):
    def __init__(self, config: RHN_ACTV1Config) -> None:
        ...
        self.dyn_scale = nn.Parameter(torch.full((self.config.hidden_size,), 1.5, dtype=self.forward_dtype))
    ...
    def _dynamic_forward(self, z_L, z_H, input_embeddings=None, log_deep_metrics=False, **seq_info) -> Tuple[
        ...
        return 0.5 * h_base + 0.5 * self.dyn_scale * h_dyn, step_l2, step_metrics
