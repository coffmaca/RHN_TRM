    ...
    def __init__(self, config: RHN_ACTV1Config) -> None:
        ...
        if self.config.mlp_t:
            ...
            self.self_attn = Attention(
                hidden_size=config.hidden_size + config.depth_enc_dim,
                head_dim=(config.hidden_size + config.depth_enc_dim) // config.num_heads,
...
class RHN_Hypernetwork(nn.Module):
    def __init__(self, config: RHN_ACTV1Config, layer_specs) -> None:
        ...
        self.input_size = (self.config.hidden_size * self.config.L_layers) + config.depth_enc_dim
        ...
        module_list = nn.ModuleList(
            [CastedLinear(self.input_size,
                          self.config.hypernet_hidden_size + self.config.depth_enc_dim,
                          bias=False)] + \
            [nn.SiLU()]
        )
        for _ in range(self.config.hypernet_hidden_depth):
            module_list.append(SwiGLU(self.config.hypernet_hidden_size + self.config.depth_enc_dim, self.config.expansion))
            module_list.append(torch.nn.RMSNorm(self.config.hypernet_hidden_size + self.config.depth_enc_dim,
                                                eps=self.config.rms_norm_eps,
                                                dtype=self.forward_dtype))

        self.hypernet_base = nn.Sequential(*module_list)

        self.output_head = CastedLinear(self.config.hypernet_hidden_size + self.config.depth_enc_dim,
    ...
    def _attention(self, inputs) -> torch.Tensor:
        ...
        q = self.perceiver_queries.view(1, Q, H, head_dim).transpose(1, 2).contiguous()
...
def get_sinusoidal_encoding(step_tensor: torch.Tensor, num_channels: int, device: torch.device,
                            dtype: torch.dtype) -> torch.Tensor:
    position = step_tensor.view(-1, 1).to(torch.float32)
    div_term = torch.exp(
        torch.arange(0, num_channels, 2, dtype=torch.float32, device=device) * -(math.log(10000.0) / num_channels))

    pe = torch.zeros(step_tensor.shape[0], 1, num_channels, dtype=torch.float32, device=device)
...
class RHN_ACTV1_Inner(nn.Module):
    ...
    def _dynamic_forward(self, z_L, z_H, input_embeddings=None, log_deep_metrics=False, global_step=None,
                         **seq_info) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        dynamic_weights, step_l2 = self.hypernet(hyper_input)
