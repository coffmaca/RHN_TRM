...
class DataProcessConfig(BaseModel):
    ...
    aug: bool = False
    # Added num_augs to specify the number of augmentations (max 8 for standard dihedral transforms)
    num_augs: int = Field(default=8, ge=1, le=8)
...
def convert_subset(set_name: str, config: DataProcessConfig):
    ...
    with open(hf_hub_download(config.source_repo, f"{set_name}.csv", repo_type="dataset"),
              newline="") as csvfile:  # type: ignore
    ...
    for inp, out in zip(tqdm(inputs), labels):
        # Dihedral transformations for augmentation (using config.num_augs instead of hardcoded 8)
        for aug_idx in range(config.num_augs if (set_name == "train" and config.aug) else 1):
    ...
    for k, v in results.items():
        np.save(os.path.join(save_dir, f"all___k.npy".replace("_k", k)), v)
...
def preprocess_data(config: DataProcessConfig):
    # Append the subsample_size to the output_dir if specified
    if config.subsample_size is not None:
        config.output_dir = f"{config.output_dir}-sub-{config.subsample_size}"
